# Copied from https://github.com/allenai/allennlp/commit/f9e2029fec4c66c751cad05be3dfb32a47dbe643
# allennlp's BeamSearch implementation for version 0.8.5
# The only change is in order to allow specification of beam size whose size is larger than the number of classes.
# Except for the first time step which now has padding to k, all the rest of the behavior is identical
...
class BeamSearch:
    ...
    def search(self,
        ...
        start_top_log_probabilities, start_predicted_classes = \
                start_class_log_probabilities.topk(min(self.beam_size, start_class_log_probabilities.size()[-1])) # argument to topk was self.beam_size

        # this block wasn't here
        if self.beam_size > start_class_log_probabilities.size()[-1]:
            x = start_top_log_probabilities
            y = start_predicted_classes
            start_top_log_probabilities = torch.cat((x, x.new_full((x.size()[0], self.beam_size - x.size()[-1]), -1e7)), dim=-1)
            start_predicted_classes = torch.cat((y, y.new_full((x.size()[0], self.beam_size - y.size()[-1]), start_predictions[0])), dim=-1)
